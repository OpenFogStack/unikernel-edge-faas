...
rm -rf registry && mkdir -p registry/{nanos-go,osv-go,linux-go,docker-go,nanos-node,osv-node,linux-node,docker-node,gvisor-go,gvisor-node}
...
memory = 512
cmdline = "--ip=eth0,%ip,%netmask --defaultgw=%gateway --nameserver=%gateway --rootfs=zfs /benchmark.so %callback"
