...
rm -rf registry && mkdir -p registry/{nanos-go,osv-go,linux-go,docker-go,nanos-node,osv-node,linux-node,docker-node,gvisor-go,gvisor-node,gvisor-kvm-go,gvisor-kvm-node}
...
cat >registry/gvisor-kvm-go/function.toml <<eof
concurrent-requests = 1
single-use = true
keepalive = $KEEPALIVE
...
cat >registry/gvisor-kvm-node/function.toml <<eof
concurrent-requests = 1
single-use = true
keepalive = $KEEPALIVE
