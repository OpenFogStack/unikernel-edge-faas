...
runsc spec -- /usr/bin/node /main.js ignore
popd
...
mv rootfs/root/main.js rootfs/main.js
runc spec
sed -i 's;"sh";"/usr/bin/node", "/main.js", "ignore";' config.json
popd
...
mv rootfs/root/benchmark rootfs/benchmark
runc spec
sed -i 's;"sh";"/benchmark", "ignore";' config.json
popd
