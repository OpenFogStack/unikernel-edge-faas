...
echo "target,language,cached,file,size" > $OUT
for f in $IN
do
  T=$(target $f)
  L=$(lang $f)
  ...
  r=$(rootfs $T)
  echo "target $T $k $r"
  K=$(cat out/$f | grep "file_read\[$k\]" | tail -n 1 | awk ' {print $2} ')
  R=$(cat out/$f | grep "file_read\[$r\]" | tail -n 1 | awk ' {print $2} ')
